...
function TestDemoApp() {
  ...
  useEffect(() => {
    console.log('TestDemoApp: Loading demo data...');
    try {
      // Try to load static demo data first
      const staticData = loadStaticDemoData();
      
      if (staticData) {
        console.log('TestDemoApp: Using static demo data', staticData);
        setDemoData(staticData);
      } else {
        // Fall back to built-in demo data
        console.warn('Static demo data not found, using fallback data');
        const fallbackData = createFallbackDemoData();
        console.log('TestDemoApp: Using fallback data', fallbackData);
  ...
  if (loading) {
    return (
      <div className="loading">
        <div>Loading Org Chart Demo...</div>
      </div>
    );
  }
